...
def APPNPModel_1Layer(in_channels, hidden_channels, out_channels, dropout=0.5, K=10, alpha=0.1):
    return Sequential('x, edge_index', [
        ##    input layer, input layer ->   hidden layer
...
def LinearChebNetModel(in_channels, hidden_channels, out_channels, dropout=0.5, K=3):
    return Sequential('x, edge_index', [
        ## input layer
        (ChebConv(in_channels, hidden_channels, K=K), 'x, edge_index -> x'),
        (BatchNorm1d(hidden_channels), 'x -> x'),
...
def MixHopGCNModel_1dropout(in_channels, hidden_channels, out_channels, dropout=0.5, powers=[0, 1, 2]):
    ## Loss 曲線平滑且緩慢下降：這是好事，代表正規化在發揮作用。
    ## Training Loss 降不下來：這代表正規化過頭了。
...
def MixHopGATModel(in_channels, hidden_channels, out_channels, dropout=0.5, powers=[0, 1, 2], heads=8):
    mix_out = hidden_channels * len(powers)
...
def MixHopGraphSAGEModel_Bottleneck(in_channels, hidden_channels, out_channels, dropout=0.5, powers=[0, 1, 2]):
    mix_out = hidden_channels * len(powers)
...
# =================================================================================
def MixHopGINModel(in_channels, hidden_channels, out_channels, dropout=0.5, powers=[0, 1, 2]):
    mix_out = hidden_channels * len(powers)
