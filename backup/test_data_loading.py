...
def test_data_loading():
    ...
    licit_count = (data.y == 0).sum().item()
    illicit_count = (data.y == 1).sum().item()
    unknown_count = (data.y == -1).sum().item()

    print(f"\nLabel distribution verification:")
    print(f"Total nodes: {total_nodes}")
    print(f"licit (0): {licit_count} ({licit_count/total_nodes*100:.1f}%)")
    ...
    # Expected from dataset description: 21% licit, 2% illicit
    expected_licit_pct = 21.0
    expected_illicit_pct = 2.0

    if abs(licit_count/total_nodes*100 - expected_licit_pct) < 1.0:
        print("✅ licit percentage matches expected ~21%")
    else:
        print("❌ licit percentage does not match expected!")
