...
class GraphFeatureEngineer:
    ...
    def compute_pagerank_features(self, alpha=0.15, max_iter=100, tol=1e-6):
        ...
        # # Prepare transition matrix weights
        # row, col = edge_index
        ...
        # return torch.stack([
        #     pr_values * num_nodes,
        #     torch.sqrt(pr_values * num_nodes),
        #     torch.log1p(pr_values * num_nodes),
        #     torch.argsort(torch.argsort(pr_values, descending=True)).float() / num_nodes
        # ], dim=1)
...
class IsolationForestBaseline:
    ...
    def __init__(self, n_estimators=100, max_samples='auto', random_state=RANDOM_SEED):
    ...
    def predict(self, data):
        ...
        X_test_scaled = self.scaler.transform(X_test).cpu().numpy()

        return (self.model.predict(X_test_scaled) == -1).astype(int), self.model.decision_function(X_test_scaled)
