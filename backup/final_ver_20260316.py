...
def gnn_objective(trial, model_name, data, batch_size, num_neighbors):
...
def train_and_test_gnn(model_name, data, batch_size, num_neighbors, best_params=None):
...
def main(dataset_name):
    ...
    if dataset_name == 'elliptic':
        ...
        batch_size=262144 #131072 2.5min/epoch, 262144 1min/epoch, 524288 2 min/epoch failed
        num_neighbors=[5, 5]
    data = data.to(device)
    ...
    for model_name, _ in gnn_models_list:
        ...
        study.optimize(lambda trial: gnn_objective(trial, model_name, data, batch_size, num_neighbors), n_trials=n_trials)
    ...
    for model_name, _ in gnn_models_list:
        ...
        gnn_val_probs, gnn_val_preds, gnn_test_probs, gnn_test_preds = train_and_test_gnn(model_name, data, batch_size, num_neighbors, best_params)
